...
import com.example.hasee.lanyademo.adapter.BlueListAdapter;
import com.example.hasee.lanyademo.bean.BlueDevice;
import com.example.hasee.lanyademo.task.BlueAcceptTask;
import com.example.hasee.lanyademo.task.BlueConnectTask;
import com.example.hasee.lanyademo.task.BlueReceiveTask;
import com.example.hasee.lanyademo.util.BluetoothUtil;
import com.example.hasee.lanyademo.widget.InputDialogFragment;
...
public class BluetoothActivity extends AppCompatActivity implements
    ...
    private ArrayList<BlueDevice> mDeviceList = new ArrayList<BlueDevice>();  //存放 蓝牙的集合
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ck_bluetooth = (CheckBox) findViewById(R.id.ck_bluetooth);  //复选框的按钮
        tv_discovery = (TextView) findViewById(R.id.tv_discovery);   //提示文本
        ...
        mBluetooth = BluetoothAdapter.getDefaultAdapter(); //获取蓝牙适配器对象
        if (mBluetooth == null) {
            Toast.makeText(this, "您的手机没有蓝牙功能", Toast.LENGTH_SHORT).show();
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        if (buttonView.getId() == R.id.ck_bluetooth) {
            if (isChecked == true) {
                beginDiscovery();
                Intent intent = new Intent(BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE); //声明一个开启蓝牙的广播
    ...
    private class BluetoothReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (action.equals(BluetoothDevice.ACTION_BOND_STATE_CHANGED)) { //蓝牙状态改变时
                //获取对方蓝牙对象
                BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        try {
            ...
            } else if (device.getBondState() == BluetoothDevice.BOND_BONDED && item.state != BlueListAdapter.CONNECTED) {
